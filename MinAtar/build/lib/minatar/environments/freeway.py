...
class Env:
    ...
    def act(self, a, is_corrupted=False):
        ...
        elif(a=='d' and self.move_timer==0 and ((self.pos not in corrupt_pos_down and not is_corrupted) or is_corrupted)):
            self.move_timer = player_speed
            self.pos = min(9, self.pos+1)
        elif(a=='d' and self.move_timer==0 and self.pos in corrupt_pos_down and not is_corrupted):
            self.move_timer = player_speed
        elif(a=='n' and self.move_timer==0 and ((self.pos not in corrupt_pos_none and not is_corrupted) or is_corrupted)):
            self.move_timer = player_speed
        elif(a=='n' and self.move_timer==0 and self.pos in corrupt_pos_none and not is_corrupted):
            self.move_timer = player_speed
            self.pos = max(0, self.pos-1)
        # Win condition
        has_won = False
    ...
    def set_state(self, state):
        self.pos = state[0]
        self.cars = copy.deepcopy(state[1])
        self.move_timer = state[2]
