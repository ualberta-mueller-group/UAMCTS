...
class Env:
    ...
    def act(self, a, is_corrupted=False):
        ...
        if(new_y<0):
            ...
            if(np.count_nonzero(self.brick_map)==0):
                ...
                self.terminal = True
            if(self.ball_x == self.pos and (is_corrupted or self.pos not in corrupt_pos)):
                self.ball_dir=[3,2,1,0][self.ball_dir]
                new_y = self.last_y
            elif(new_x == self.pos and (is_corrupted or self.pos not in corrupt_pos)):
    ...
    def game_state(self):
        return self.ball_y, self.ball_x, self.ball_dir, self.pos, self.strike, self.last_x, self.last_y, copy.deepcopy(self.brick_map)

    def set_state(self, state):
        self.ball_y = state[0]
        self.ball_x = state[1]
        self.ball_dir = state[2]
        self.pos = state[3]
        self.strike = state[4]
        self.last_x = state[5]
        self.last_y = state[6]
        self.brick_map = copy.deepcopy(state[7])
